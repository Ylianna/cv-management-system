        ...
        <div className="container py-4" style={{maxWidth: '850px'}}>
            ...
            <div className="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                <div className="d-flex gap-2">
                    ...
                    <div className="btn-group btn-group-sm">
                        <button className={`btn ${activeView === 'cv' ? 'btn-dark' : 'btn-outline-dark'}`}
                                onClick={() => setActiveView('cv')}>{t('tab_cv_view')}</button>
                        <button className={`btn ${activeView === 'chat' ? 'btn-dark' : 'btn-outline-dark'}`}
                                onClick={() => setActiveView('chat')}>{t('tab_discussion')}</button>
                    </div>
                </div>
                {activeView === 'cv' ? (
                        <div className="card shadow-lg border-0 p-5 bg-white text-dark rounded-3">
                            <div>
                                {cv?.isPublished ? (
                                    <span
                                        className="badge bg-success p-2 d-flex align-items-center gap-1">{t('status_published')}</span>
                                ) : (
                                    <button className="btn btn-sm btn-primary" disabled={!isPublishable}
                                            onClick={handlePublish}>{t('btn_publish_cv')}</button>
                                )}
                            </div>
                        </div>
                    )
            ...
            <div className="card shadow-lg border-0 p-5 bg-white text-dark rounded-3">
                <div className="border-bottom pb-4 mb-4">
                    ...
                    <div
                        className="badge bg-primary-subtle text-primary mt-2">{t('cv_for_position', {title: position?.title})}</div>
                </div>
                ...
                <div className="mb-4">
                    ...
                    <span className="fw-semibold small text-dark">
                      {attr.name} {isRequired && <span className="text-danger">*</span>}
                                        ...
                                        {isEmpty && <span className="text-danger small d-flex align-items-center gap-1"><AlertCircle
                                            size={12}/> {t('field_required_warning')}</span>}
                                    </div>
                                    ...
                                    {attr.type === 'DROPDOWN' ? (
                                        ...
                                        >
                                            ...
                                            {attr.options?.map((opt: string) => <option key={opt}
                        ...
                        projects.map(p => (
                            <div key={p.id} className="mb-4">
                                <div className="d-flex justify-content-between align-items-baseline">
                                    ...
                                    <small
                                        className="text-muted fw-medium">{new Date(p.startDate).toLocaleDateString()} — {new Date(p.endDate).toLocaleDateString()}</small>
                                </div>
                                <div className="small text-secondary mt-1">
                                    <ReactMarkdown>{p.description}</ReactMarkdown>
                                </div>
                                <div className="d-flex gap-1 mt-1">
                                    {p.tags?.map((t: string) => <span key={t}
                                                                      className="badge bg-light text-muted border small">{t}</span>)}
                                </div>
                            </div>
                        ))
                    )}
