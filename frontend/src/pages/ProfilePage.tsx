...
export const ProfilePage: React.FC = () => {
    ...
    const loadAllData = () => {
        ...
        fetch(`${BACKEND_URL}/api/profile/${profileId}/attributes`)
            .then(res => res.json())
            .then(data => {
                if (Array.isArray(data)) setUserValues(data);
            });
    };
    ...
    const handleSaveProject = async () => {
        if (!newProject.name || !newProject.startDate || !newProject.endDate) {
            toast.error(t('toast_project_title_dates'));
            return;
        }
        try {
            ...
            setActiveError('err_project_save_failed');
        }
    };
    ...
    return (
        <div className="container py-4" style={{maxWidth: '850px'}}>
            ...
            <div
                className="d-flex justify-content-between align-items-center mb-4 p-3 rounded border shadow-sm bg-light-subtle">
                <h4 className="fw-bold m-0 d-flex align-items-center gap-2">
                    <Save size={20} className="text-primary"/>
                    {t('profile_title')}
                </h4>
                <div>
                    {syncStatus === 'saving' && <span className="badge bg-warning text-dark p-2"><RefreshCw size={14}
                                                                                                            className="spinner-border spinner-border-sm border-0"/> {t('status_saving')}</span>}
                    {syncStatus === 'saved' &&
                        <span className="badge bg-success p-2"><CheckCircle size={14}/> {t('status_saved')}</span>}
                    {syncStatus === 'conflict' &&
                        <span className="badge bg-danger p-2"><AlertTriangle size={14}/> {t('status_conflict')}</span>}
                </div>
            </div>
                ...
                <div className="card-body p-4">
                    <div className="row g-3">
                        <div className="col-md-6">
                            ...
                            <input type="text" className="form-control" placeholder={t('placeholder_firstname')}
                        ...
                        <div className="col-md-6">
                            ...
                            <input type="text" className="form-control" placeholder={t('placeholder_lastname')}
                        ...
                        <div className="col-12">
                            ...
                            <input type="text" className="form-control" placeholder={t('placeholder_location')}
            ...
            <div className="border-top pt-4 mb-5">
                <h4 className="fw-bold mb-3 d-flex align-items-center gap-2"><Info size={22}
                                                                                   className="text-primary"/> {t('section_info_title')}
                </h4>
                ...
                <div className="row g-3 mb-4">
                    <div className="col-md-6">
                        <div className="input-group input-group-sm">
                            ...
                            <input type="text" className="form-control" placeholder={t('placeholder_search_prefix')}
                ...
                {recentlyUsed.length > 0 && (
                    <div className="mb-3 p-2 bg-light-subtle border rounded small">
                        <span className="text-muted fw-bold d-inline-flex align-items-center gap-1 me-2"><Clock
                            size={12}/> {t('recent_fields')}</span>
                        {recentlyUsed.map(r => (
                            <button key={r.id} type="button" className="btn btn-xs btn-outline-secondary me-1 py-0 px-2"
                                    onClick={() => handleAddAttributeToProfile(r)}>+ {r.name}</button>
                        ))}
                    </div>
                )}

                <div className="mb-4 d-flex flex-wrap gap-1 p-2 border rounded bg-light"
                     style={{maxHeight: '120px', overflowY: 'auto'}}>
                    {library.map(attr => (
                        <button key={attr.id} type="button"
                                className="btn btn-sm btn-light border d-inline-flex align-items-center gap-1"
                                onClick={() => handleAddAttributeToProfile(attr)}>
                            <Plus size={12}/> <span className="fw-semibold text-dark">{attr.name}</span> <span
                            className="text-muted small">({attr.type})</span>
                        </button>
                    ))}
                </div>
                ...
                <div className="space-y-3 mb-4">
                    {userValues.map(item => (
                        <div key={item.attributeId}
                             className="card shadow-sm p-3 mb-2 border-start border-primary border-3">
                                    ...
                                    {item.attribute.type === 'BOOLEAN' ? (
                                        <div className="form-check">
                                            <input type="checkbox" className="form-check-input"
                                                   ...
                                                   onChange={e => handleSaveAttributeValue(item.attributeId, String(e.target.checked))}/>
                                            <label className="form-check-label">{t('boolean_yes')}</label>
                                        </div>
                                    ) : item.attribute.type === 'DROPDOWN' ? (
                                        <select className="form-select form-select-sm" value={item.value}
                                                onChange={e => handleSaveAttributeValue(item.attributeId, e.target.value)}>
                                            <option value="">{t('select_value_placeholder')}</option>
                                            {item.attribute.options?.map(opt => <option key={opt}
                                                                                        value={opt}>{opt}</option>)}
                                        </select>
                                    ) : (
                                        <input type="text" className="form-control form-control-sm"
                                               placeholder={t('placeholder_input_value', {type: item.attribute.type})}
                                               value={item.value}
                                               onChange={e => setUserValues(prev => prev.map(v => v.attributeId === item.attributeId ? {
                                                   ...v,
                                                   value: e.target.value
                                               } : v))}
                                               onBlur={e => handleSaveAttributeValue(item.attributeId, e.target.value)}/>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            <div className="border-top pt-4">
                <h4 className="fw-bold mb-4 d-flex align-items-center gap-2"><Calendar size={22}
                                                                                       className="text-success"/> {t('section_projects_title')}
                </h4>
                <div className="card shadow-sm border-success-subtle mb-4">
                    <div
                        className="card-header bg-success-subtle text-success-emphasis fw-bold">{t('card_add_project_title')}</div>
                    <div className="card-body p-4">
                        <div className="row g-3">
                            <div className="col-12">
                            ...
                            <div className="col-md-6">
                                ...
                                <input type="date" className="form-control" value={newProject.startDate}
                                       onChange={e => setNewProject({...newProject, startDate: e.target.value})}/>
                            </div>
                            <div className="col-md-6">
                                <label className="form-label fw-semibold">{t('label_end_date')}</label>
                                <input type="date" className="form-control" value={newProject.endDate}
                            ...
                            <div className="col-12 position-relative">
                                ...
                                <div className="d-flex flex-wrap gap-1 mb-2">
                                    {newProject.tags.map(t => (
                                        <span key={t}
                                              className="badge bg-secondary p-2 d-flex align-items-center gap-1">
                      {t} <Trash2 size={12} style={{cursor: 'pointer'}} onClick={() => setNewProject({
                                            ...newProject,
                                            tags: newProject.tags.filter(tag => tag !== t)
                                        })}/>
                    </span>
                                    ))}
                                </div>
                                <div className="input-group">
                                    <input type="text" className="form-control form-control-sm"
                                           placeholder={t('placeholder_enter_tag')} value={tagInput}
                                           onChange={e => setTagInput(e.target.value)}/>
                                    <button className="btn btn-outline-secondary btn-sm" type="button"
                                            onClick={() => handleAddTag(tagInput)}>{t('btn_add_tag')}</button>
                                </div>
                                    ...
                                    <div className="list-group shadow-sm mt-1 position-absolute z-3 w-100">
                                        {allExistingTags.filter(t => t.toLowerCase().startsWith(tagInput.toLowerCase())).map(t => (
                                            <button key={t} type="button"
                                                    ...
                                                    onClick={() => handleAddTag(t)}>{t}</button>
                                        ))}
                                    </div>
                                )}
                            </div>
                            <div className="col-12">
                                ...
                                {previewMarkdown ? (
                                    ...
                                    <textarea className="form-control" rows={4} value={newProject.description}
                            ...
                            <div className="col-12 text-end">
                                <button type="button" className="btn btn-success d-inline-flex align-items-center gap-1"
                                        onClick={handleSaveProject}><Plus size={16}/> {t('btn_save_project')}</button>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div className="card-body">
                                <div className="d-flex justify-content-between align-items-start">
                                    ...
                                    <button className="btn btn-sm btn-outline-danger"
                                            onClick={() => p.id && handleDeleteProject(p.id)}><Trash2 size={14}/>
                                    </button>
                                </div>
                                ...
                                <div className="d-flex flex-wrap gap-1 mt-2">
                                    {p.tags?.map(t => <span key={t}
                                                            className="badge bg-light text-dark border">{t}</span>)}
                                </div>
                            </div>
                        </div>
                    ))}
